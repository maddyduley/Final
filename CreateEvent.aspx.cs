        ...
        {
            {
                try
                {
                    using (SqlConnection sc = new SqlConnection(WebConfigurationManager.ConnectionStrings["Lab3"].ConnectionString.ToString()))
                    {
                        ...
                        cmd.Connection = sc;
                        cmd.CommandText = "INSERT INTO Event VALUES (@Title,@Date,@Desc,@Loc," + Session["Member"].ToString() + ")";

                        cmd.Parameters.Add(new SqlParameter("@Title", HttpUtility.HtmlEncode(Named.Text)));
                        cmd.Parameters.Add(new SqlParameter("@Date", HttpUtility.HtmlEncode(Datetxt.Text)));
                        cmd.Parameters.Add(new SqlParameter("@Desc", HttpUtility.HtmlEncode(EventDesc.Text)));
                        cmd.Parameters.Add(new SqlParameter("@Loc", HttpUtility.HtmlEncode(EvenLoc.Text)));
                                ...
                                for (int i = 1; i <= Convert.ToInt32(Session["CountingStudents"]) + 1; i++)
                                {
                                    ...
                                    if (Session["resulted"].ToString() == "True")
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            String contentbody = "Dear " + Session["Firstss"].ToString() + " " + Session["Lastss"].ToString() + System.Environment.NewLine + "A new event has been added." + System.Environment.NewLine + "Name:" + Named.Text + System.Environment.NewLine + "Descrption:" + EventDesc.Text + System.Environment.NewLine + "Date:" + Datetxt.Text + System.Environment.NewLine + "Location:" + EvenLoc.Text + System.Environment.NewLine;
