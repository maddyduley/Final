...
namespace Lab3
{
    public partial class StudentSearch : System.Web.UI.Page
    {
        ...
        protected void Search4Student_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT StudentID, FirstName, LastName, Email, PhoneNumber, UniversityYear, Major, GradYear, UserName, Files, Image FROM Student WHERE FirstName=@Serch OR LastName=@Serch OR FirstName LIKE '%' + @Serch + '%' OR LastName LIKE '%' + @Serch + '%' OR FirstName + ' ' +LastName=  @Serch");
            cmd.Connection = con;
